  ...
  if (!is_initialized_) {
    ...
    if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      */
      ...
      //initialize px and py using directly readings from laser
      px = meas_package.raw_measurements_[0];
      py = meas_package.raw_measurements_[1];

    }

    // If initial values are < 0.0001 px is set to default 0.1 and increase in uncertainty of measurment
...
void UKF::Prediction(double delta_t) {
  ...
  ********************************************/
  ...
  P_.fill(0.0);
  for (int i = 0; i < n_sig_; i++) {  //iterate over 15 sigma points
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  /*******************************************
  Step 1: Using original predicted sigma points Xsig_pred_ and
  transfor them into measurement space r, phi, r_dot
  ********************************************/
  ...
  for (int i = 0; i < n_sig_; i++) {  //2n+1 simga points
    ...
    // conversion to measurement model
    // Zsig_radar_ is a 3 x 15 matrix with 3 measurement and 15 sigma points as original
    if(p_x == 0 && p_y == 0){
      Zsig_radar_(0,i) = 0;   //r
  ...
  /*******************************************
  Step 2: Using Zsig_radar_ sigma points to predict mean and covariance for measurement model
  ********************************************/
  ...
  //measurement noise is purely additive
  S = S + R_radar_;

  /*******************************************
  Step 3: UKF update by
  ...
  ********************************************/
  ...
  //by running through all sigma points
  for (int i = 0; i < n_sig_; i++) {  //2n+1 simga points

    //residual difference between predicted radar and radar sigma points
    VectorXd z_diff = Zsig_radar_.col(i) - z_pred;
    //angle normalization
    while (z_diff(1)> M_PI) z_diff(1)-=2.*M_PI;
    while (z_diff(1)<-M_PI) z_diff(1)+=2.*M_PI;

    //state difference between state x_ and predicted state x_ sigma points
    VectorXd x_diff = Xsig_pred_.col(i) - x_;
    //angle normalization
    while (x_diff(3)> M_PI) x_diff(3)-=2.*M_PI;
    while (x_diff(3)<-M_PI) x_diff(3)+=2.*M_PI;
